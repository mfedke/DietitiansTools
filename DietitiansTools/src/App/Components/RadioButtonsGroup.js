...
export default class RadioGroup extends Component {
  ...
  validate(data) {
    ...
    data.map(e => {
      e.color = e.color ? e.color : '#86cacb';
  ...
  render() {
    ...
    return (
      <TouchableOpacity
        ...
        <View style={{flexDirection: 'row', borderWidth: 1, borderColor: '#86cacb', borderRadius: 10, paddingTop: '1%', paddingBottom: '6%', paddingLeft: '3%', paddingRight: '3%'}}>
          <View
            style={[
              styles.border,
              {
          ...
          <Text style={[{ alignSelf: 'center', fontSize: 16, color: 'black', fontWeight: data.selected ? 'bold' : 'normal'}, margin]}>{data.label}</Text>
        </View>
      </TouchableOpacity>
    );
  }
